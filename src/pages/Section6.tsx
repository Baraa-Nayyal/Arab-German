import React from "react";

const Section6 = () => {
  React.useEffect(() => {
    window.scrollTo(0, 0);
  }, []);
  return (
    <div className="h-[50vh] relative flex justify-center items-center">
      <div className="twoOverlay h-full w-full absolute z-10"></div>
      <div className="bgSection6 bg " />
      <div className="z-20 relative text-center flex flex-col px-10">
        <h1 className="inline-block">Die Finanzierung des Vereins</h1>
        <h3 className="inline-block">
          erfolgt durch Sach- und Geldspenden und Zuwendungen. Spender können
          offizielle Quittungen erhalten.
        </h3>
      </div>
    </div>
  );
};

export default Section6;
